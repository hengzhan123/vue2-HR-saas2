<template>
   ...
   <div class="body2">
    ...
    <el-timeline-item
      ...
      :key="index"
      :timestamp="activity.handleTime | dateFormat "
...
import {DetailsAPI,DetailAPT, }from "@/api"
    export default { 
        data() {
      return {
        reverse:true,
    ...
    methods:{
      //明细
      async Details(){
         const res =await DetailsAPI(this.$route.params.processId)
         this.activities=res.data
      },
      async Detail(){
         const res =await DetailAPT(this.$route.params.processId)
          this.DetailsList=res.data
          this.processName=res.data.processName
          this.DetaList=JSON.parse(res.data.procData)
      }
    },
    created(){
      this.Details()
